...
bool Library::is_parent_sub_path( const path& parent, const path& sub )
{
...
void Library::add_book( const path& book_path, const std::string& folder_name )
{
    ...
    if ( boost::filesystem::exists( new_book_path ) )
    {
        ...
        if ( is_size_equal( book_path, new_book_path ) )
        {
            std::cout << "文件大小相同，删除文件：" << book_path.string() << std::endl;
            remove_book( book_path );
            return;
        }
...
void Library::add_directory( const path& dir )
{
    ...
    {
        std::cout << "文件夹不存在：" << dir << std::endl;
        return;
    }

    if ( ! boost::filesystem::is_directory( dir ) )
    {
        std::cout << "不是文件夹：" << dir << std::endl;
        return;
    }
...
void Library::create_index( const char top_class_name )
{
    ...
    if ( m_it == m_end )
    {
        m_current_top_class = 'z';
        std::cout << "索引完成，共 " << m_file_count << " 个文件，" << m_folder_count << " 个文件夹。" << std::endl;
    }
}


bool Library::is_size_equal( const path& lhs, const path& rhs )
{
...
void Library::remove_book( const path& book_path )
{
    boost::system::error_code ec;
